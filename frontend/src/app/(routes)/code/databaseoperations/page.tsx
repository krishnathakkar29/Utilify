...
const DashboardPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(`https://reception-poultry-ec-booking.trycloudflare.com/generate-sql`, {
        db_url: dbUrl,
        query: query,
      });
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen gap-4 p-6 bg-gray-900 text-white">
      ...
      />
      <Button onClick={handleSubmit} disabled={loading} className="bg-purple-700 text-white">
                      ...
                      {sqlResult.results.map((row, i) => (
                        ...
                        >
                          {Object.values(row).map((val, j) => (
                            <td key={j} className="px-3 py-2 break-words max-w-[300px]">
                              {val?.toString()}
                            </td>
                          ))}
                        </tr>
