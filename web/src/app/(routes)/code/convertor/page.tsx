...
function page() {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      toast.error(
        `Validation failed: ${error instanceof Error ? error.message : "An unknown error occurred"}`
      );
    } finally {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      console.log(error);
      toast.error(
        `Conversion failed: ${error instanceof Error ? error.message : "An unknown error occured"}`
      );
    } finally {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-center mb-2">
        Text Format Validator & Converter
      </h2>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <div className="flex flex-col space-y-1.5">
                ...
                <p className="text-sm text-muted-foreground">
                  Enter your text to validate or convert
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <Select
                  value={inputFormat}
                  onValueChange={(value: DataFormat) => setInputFormat(value)}
                >
        ...
        <div className="flex justify-center mt-6 space-x-4">
          ...
          <TabsContent value="convert" className="mt-0">
            <Button
              ...
              disabled={
                isConverting || !inputText || inputFormat === outputFormat
              }
              className="px-8"
            >
