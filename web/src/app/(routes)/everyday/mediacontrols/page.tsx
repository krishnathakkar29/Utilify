...
export default function MediaToolsPage() {
  const [action, setAction] = useState<
    "" | "convert_video" | "convert_audio" | "trim_video" | "trim_audio"
  >("");
  ...
  return (
    <Card className="w-full min-h-screen bg-black text-white p-6 flex flex-col items-center gap-6">
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        {["convert_video", "convert_audio", "trim_video", "trim_audio"].map(
          (type) => (
            <button
              key={type}
              onClick={() => {
      ...
      {/* Output Section */}
      {loading && (
        <p className="text-neutral-400 text-sm mt-4">Processing...</p>
      )}
